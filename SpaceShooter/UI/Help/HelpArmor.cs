...
{
    public class HelpArmor : SysMenu
    {
        int WINDOWWIDTH = 900;
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            Helpers.DrawOutline(FrameworkCore.Gothic, Resource.HelpArmorTitle, titlePos, titleColor, darkColor,
                0, Vector2.Zero, 1);
            ...
            Vector2 offset = new Vector2(WINDOWWIDTH / 2 + 20, 0);

            int turnVec = (int)FrameworkCore.Serif.MeasureString(descTxt).Y;
            titlePos.Y += turnVec + 80;


            Color phaseColor = Color.Goldenrod;
            phaseColor = Color.Lerp(Helpers.transColor(phaseColor), phaseColor, Transition);
            FrameworkCore.SpriteBatch.DrawString(FrameworkCore.SerifBig, Resource.HelpArmorTopTitle, titlePos,
                phaseColor, 0, Vector2.Zero, 0.9f, SpriteEffects.None, 0);
            FrameworkCore.SpriteBatch.DrawString(FrameworkCore.SerifBig, Resource.HelpArmorBottomTitle,
                titlePos + offset,
                phaseColor, 0, Vector2.Zero, 0.9f, SpriteEffects.None, 0);
            ...
            DrawTextBlock(titlePos, new string[] { descAction });

            DrawTextBlock(titlePos + offset, new string[] { descOrder });
        }



        string descTxt = string.Empty;
        string descAction = string.Empty;
        string descOrder = string.Empty;

        public override void Activate()
        {
            descTxt = Helpers.StringWrap(FrameworkCore.Serif, Resource.HelpArmorDesc, WINDOWWIDTH);
            descAction = Helpers.StringWrap(FrameworkCore.Serif, Resource.HelpArmorTop, WINDOWWIDTH/2 - 10);
            descOrder = Helpers.StringWrap(FrameworkCore.Serif, Resource.HelpArmorBottom, WINDOWWIDTH/2 - 10);
