...
namespace SpaceShooter
{
    ...
    public abstract class ParticleSystem : DrawableGameComponent
    {
        ...
        public void AddParticle(Vector3 position, Vector3 velocity)
        {
            ...
            particles[firstFreeParticle].Position.X = position.X + settings.MinPositionOffset + ((settings.MaxPositionOffset - settings.MinPositionOffset) * (float)random.NextDouble());
            particles[firstFreeParticle].Position.Y = position.Y + settings.MinPositionOffset + ((settings.MaxPositionOffset - settings.MinPositionOffset) * (float)random.NextDouble());
            particles[firstFreeParticle].Position.Z = position.Z + settings.MinPositionOffset + ((settings.MaxPositionOffset - settings.MinPositionOffset) * (float)random.NextDouble());
